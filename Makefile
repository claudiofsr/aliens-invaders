...
install: all
	...
	@for sz in 48x48 96x96 128x128 256x256; do \
		install -c -m 0644 "$(assets)/icons/hicolor/$$sz/apps/aliens-invaders.png" "$(DESTDIR)$(datadir)/icons/hicolor/$$sz/apps/aliens-invaders.png"; \
	done
	...
	@if [ -n "$$SUDO_USER" ]; then \
		chown -R "$$SUDO_USER" "$(builddir)" "$(BAKE_TOOL)" "$(EMBEDDED_SRC)" 2>/dev/null || true; \
	fi
...
uninstall:
	...
	@for sz in 48x48 96x96 128x128 256x256; do \
		rm -f "$(DESTDIR)$(datadir)/icons/hicolor/$$sz/apps/aliens-invaders.png"; \
	done
