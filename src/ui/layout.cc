...
float GetShowcaseSpriteSize(float win_h, bool is_fullscreen,
  ...
  if (is_fullscreen) {
    if (win_h <= 1080.0f) {
      ...
      scale_factor = std::clamp(1.0f + 1.0f * ((win_h - 1080.0f) / 1080.0f), 1.0f, 2.0f);
    }
  } else {
    if (win_h <= 760.0f) {
      scale_factor = 0.70f;
...
float TacticalCard::ComputeHeight(float scale) const noexcept {
  ...
  const float header_h = top_pad + title_font + sub_gap + sub_font + items_gap;
  return header_h + static_cast<float>(items_.size()) * item_step + bottom_pad;
}
...
void TacticalCard::Draw(SDL_Renderer* renderer, float x, float y, float width, float scale) const {
  ...
  Gfx::Inst().DrawModernText(
      Coord(static_cast<int32_t>(std::round(x + pad_x)),
  ...
  const float header_total_h = top_pad + title_font + 4.0f * scale + sub_font + 12.0f * scale;
  float cur_item_y = y + header_total_h;

  for (const auto& item : items_) {
    Gfx::Inst().DrawModernText(
        Coord(static_cast<int32_t>(std::round(x + pad_x + 6.0f * scale)),
              static_cast<int32_t>(std::round(cur_item_y))),
        item.label, item.label_r, item.label_g, item.label_b, item_font);

    float desc_x;
    if (label_col_w_ > 0.0f) {
      desc_x = x + pad_x + (label_col_w_ * scale);
    } else {
      const float lw = Gfx::Inst().GetTextWidth(item.label, item_font);
      desc_x = x + pad_x + 12.0f * scale + lw;
    }

    Gfx::Inst().DrawRegularText(
        Coord(static_cast<int32_t>(std::round(desc_x)),
              static_cast<int32_t>(std::round(cur_item_y))),
...
// ==============================================================================
// TelemetryPlate Implementation (Mathematical Optical Centering)
// ==============================================================================

void TelemetryPlate::Draw(SDL_Renderer* rend, float x, float y, float w, float h, float s, float value_col_x) const {
  if (!rend) return;

  // 1. Semi-transparent glassmorphic background & glowing neon frame
  ...
  // 2. Proportional typography metrics
  const float font_size = Typography::SectionHeader(s); // 27.0f * s

  // 3. Mathematical Optical Centering:
  // In TrueType rasterization, glyph caps sit ~0.72*font_size above baseline.
  // To align the optical vertical midpoint of text with the geometric midpoint of the box:
  // y_text = (y + h * 0.5f) - (0.64f * font_size).
  const float box_mid_y = y + (h * 0.5f);
  const float text_y = box_mid_y - (0.64f * font_size);

  const float pad_x = 26.0f * s;

  // Left Label
  Gfx::Inst().DrawModernText(
      Coord(static_cast<int32_t>(std::round(x + pad_x)),
            static_cast<int32_t>(std::round(text_y))),
      label_, label_r_, label_g_, label_b_, font_size);

  // Right Aligned Value
  Gfx::Inst().DrawModernText(
      Coord(static_cast<int32_t>(std::round(x + value_col_x * s)),
            static_cast<int32_t>(std::round(text_y))),
      value_, value_r_, value_g_, value_b_, font_size);
}
