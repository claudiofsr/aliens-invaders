...
Telemetry::Telemetry() {
  ...
  // Start asynchronous background thread for system polling
  worker_thread_ = std::thread(&Telemetry::WorkerLoop, this);
}
...
void Telemetry::WorkerLoop() {
  while (running_.load(std::memory_order_relaxed)) {
    // Sleep asynchronously for 500ms without touching the main thread
    std::this_thread::sleep_for(std::chrono::milliseconds(500));
    if (!running_.load(std::memory_order_relaxed)) break;

    // Only perform kernel I/O queries if telemetry is visible
...
// Strictly pure memory-based presentation: Zero I/O and zero syscalls on main thread
