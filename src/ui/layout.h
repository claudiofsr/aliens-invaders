 ...
 * @brief SINGLE SOURCE OF TRUTH (SSOT) for typography dimensions in Aliens Invaders.
 */
struct FontDesignTokens {
...
class Typography {
 public:
  ...
  [[nodiscard]] static float Title() noexcept { return Title(AutoScale()); }
  [[nodiscard]] static float Subtitle() noexcept { return Subtitle(AutoScale()); }
  [[nodiscard]] static float SectionHeader() noexcept { return SectionHeader(AutoScale()); }
  [[nodiscard]] static float CardTitle() noexcept { return CardTitle(AutoScale()); }
  [[nodiscard]] static float ItemName() noexcept { return ItemName(AutoScale()); }
  [[nodiscard]] static float Body() noexcept { return Body(AutoScale()); }
  [[nodiscard]] static float ItemDesc() noexcept { return ItemDesc(AutoScale()); }
  [[nodiscard]] static float Detail() noexcept { return Detail(AutoScale()); }
  [[nodiscard]] static float FooterNav() noexcept { return FooterNav(AutoScale()); }
  [[nodiscard]] static float FooterCopy() noexcept { return FooterCopy(AutoScale()); }
};
...
class TacticalCard {
 ...
 public:
  ...
  [[nodiscard]] float ComputeHeight(float scale) const noexcept;
 ...
 * @brief Sci-fi telemetry badge/plate with optical vertical centering.
 * Symmetrically centers labels and values inside neon borders.
 */
class TelemetryPlate {
  std::string label_;
  std::string value_;
 ...
 public:
  TelemetryPlate(std::string label, std::string value,
...
/**
 * @class ListLayout
 * @brief Reusable C++20 layout engine implementing mathematical Flexbox space-evenly.
 */
class ListLayout {
