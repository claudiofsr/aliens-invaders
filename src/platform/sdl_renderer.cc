...
class FontEngine {
 ...
 public:
  ...
  GlyphInfo* GetOrCreateGlyph(SDL_Renderer* renderer, char c, int size,
    ...
    const size_t row_idx_sz = static_cast<size_t>(clamped_sz - MIN_SZ);
    GlyphInfo& glyph = cache[row_idx_sz][uc];
    glyph.width = width;
    glyph.height = height;
...
int Pix::Width() const noexcept {
  return static_cast<int>(std::round(static_cast<float>(base_dim_.x) * Gfx::Inst().Scale()));
}

int Pix::Height() const noexcept {
  return static_cast<int>(std::round(static_cast<float>(base_dim_.y) * Gfx::Inst().Scale()));
}
...
Gfx::Gfx() {
  ...
  GameRules::Fleet::InvalidateMetrics(scale_);
