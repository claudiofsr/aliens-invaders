...
struct FlightPath {
  ...
  static std::vector<Vec2f> GenerateSmoothSplineF(
    ...
    uniform_pts.reserve(fine_pts.size() + 16);
  ...
  static FlightPath RandomEdge(RandomStream& rng) {
    int edge = rng.UniformInt(0, 2);
    float sx, sy;
    if (edge == 0) { sx = 80.0f + static_cast<float>(rng.UniformInt(0, 863)); sy = -60.0f; }
    else if (edge == 1) { sx = -60.0f; sy = 50.0f + static_cast<float>(rng.UniformInt(0, 449)); }
    else { sx = 1084.0f; sy = 50.0f + static_cast<float>(rng.UniformInt(0, 449)); }

    float tx = 200.0f + static_cast<float>(rng.UniformInt(0, 623));
    float ty = 250.0f + static_cast<float>(rng.UniformInt(0, 2) * 50);
    float mx1 = sx + (tx - sx) * 0.33f + static_cast<float>(rng.UniformInt(-100, 99));
    float my1 = sy + (ty - sy) * 0.33f + static_cast<float>(rng.UniformInt(-50, 49));
    float mx2 = sx + (tx - sx) * 0.66f + static_cast<float>(rng.UniformInt(-100, 99));
    float my2 = sy + (ty - sy) * 0.66f + static_cast<float>(rng.UniformInt(-50, 49));
  ...
  static FlightPath ZigZag(float start_x, float start_y, float step_y,
    ...
    pts.reserve(static_cast<std::size_t>(steps) + 1u);
    pts.emplace_back(start_x, start_y);

    for (int i = 0; i < steps; ++i) {
      const float x = (i % 2 == 0) ? x_amp : 0.0f;
      const float y = start_y + static_cast<float>(i + 1) * step_y;
