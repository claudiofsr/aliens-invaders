...
class CombatSession {
 ...
 public:
  ...
  ~CombatSession() {
    if (pause_texture_) {
      SDL_DestroyTexture(pause_texture_);
 ...
 private:
  ...
  bool TickFixedPhysics() {
    ...
    if (!phase_transition_started_ && aliens_->Finished()) {
      ...
      constexpr double kHyperspaceWarpDurationSec = static_cast<double>(GameRules::Progression::kHyperspaceWarpDurationSec);
  ...
  void RenderPauseUI(SDL_Renderer* rend, float win_w, float win_h, float s) {
    ...
    plates.emplace_back("T / [Y]", "Toggle Vessel: " + ship_str, 0, 255, 240)
        .SetLabelColor(255, 230, 100)
        .SetFontSizes(Typography::ItemName(s), Typography::Subtitle(s));
  ...
  bool ExecutePauseMenu(double current_time) {
    ...
    if (!pause_texture_ || pause_tex_w_ != win_w || pause_tex_h_ != win_h) {
      if (pause_texture_) SDL_DestroyTexture(pause_texture_);
      pause_texture_ = SDL_CreateTexture(rend, SDL_PIXELFORMAT_RGBA32,
                                         SDL_TEXTUREACCESS_TARGET, win_w, win_h);
      if (pause_texture_) {
        SDL_SetTextureBlendMode(pause_texture_, SDL_BLENDMODE_BLEND);
      }
      pause_tex_w_ = win_w;
      pause_tex_h_ = win_h;
      pause_dirty_ = true;
  ...
  void ExecuteDebriefing() {
    ...
    if (rend) {
      debrief_overlay_tex = SDL_CreateTexture(rend, SDL_PIXELFORMAT_RGBA32,
                                              SDL_TEXTUREACCESS_TARGET, win_w, win_h);
      if (debrief_overlay_tex) {
        SDL_SetTextureBlendMode(debrief_overlay_tex, SDL_BLENDMODE_BLEND);
    ...
    if (debrief_overlay_tex) {
      SDL_DestroyTexture(debrief_overlay_tex);
      debrief_overlay_tex = nullptr;
    }
  }
