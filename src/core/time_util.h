...
class FramePacingEngine final {
  ...
  // Spiral-of-death guard: maximum simulation steps per visual frame.
  // If the game lags (e.g., disk I/O spike), we cap catch-up to avoid
  // freezing the game loop. Standard practice (Gaffer on Games, Valve).
  // Spiral-of-death guard: bounded accumulator strictly capped at 2 steps (33.3 ms)
 ...
 private:
  ...
  // 0.25 ms VBlank jitter slack window: guarantees rock-solid 1 physics step per frame on 60 Hz displays
  static constexpr std::uint64_t kJitterSlackNs = 250'000ULL;
