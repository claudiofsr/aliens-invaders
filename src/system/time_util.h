...
class FramePacingEngine final {
 ...
 private:
  static constexpr std::uint64_t kFixedStepNs = 16'666'667ULL;
  static constexpr std::uint64_t kMaxFrameDeltaNs = 250'000'000ULL;
  static constexpr std::uint64_t kMaxAccumulatorNs =
      kFixedStepNs * 2ULL;

  std::uint64_t previous_ns_{0};
  std::uint64_t accumulator_ns_{0};
  std::uint64_t target_frame_ns_{kFixedStepNs};
